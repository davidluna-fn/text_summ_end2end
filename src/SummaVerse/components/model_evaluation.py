...
from SummaVerse.entity import ModelEvaluationConfig
import torch
import pandas as pd
...
class ModelEvaluation:
    ...
    def calculate_metric_on_test_ds(self, dataset, metric, model, tokenizer, 
        ...
        for article_batch, target_batches in tqdm(
            ...
            ''' penalty for length penalty ensure that the model not generate sequences that are too long '''

            # Finally, we decode the generated text
            # Remplace the token, and add the decoded text with the references to the metric.
    ...
    def evaluate(self):
        ...
        score = self.calculate_metric_on_test_ds(dataset_samsum_pt['test'][0:10], rouge_metric, model_pegasus, tokenizer, batch_size=2, column_text='article', column_summary='highlights', device=device)
